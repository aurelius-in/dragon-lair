...
// Game reset
function resetGame() { // Reset dragon and obstacles
...
function collisionDetected(dragon, obstacle) { // Reduce the boundary by 5% of the width and 10% of the height
...
function levelEnd() { // Gradually increase the scale for zooming effect
    dragon.scale += 0.005;
...
update();
draw();
if (collisionDetected(dragon, obstacle)) { // Collision detected
...
if (gameStarted) { // If tapping, update the frame based on framesPerFlap
    if (gameLoopCounter % framesPerFlap === 0) {
        currentFrame = (currentFrame + 1) % dragonImages.length;
    }
} else { // If not tapping, change the frame every 60 iterations (1 second)
    if (gameLoopCounter % framesPerFlap === 0) {
        currentFrame = (currentFrame + 1) % dragonImages.length;
    }
}
requestAnimationFrame(gameLoop);}gameLoop();window.onload = () => {
setTimeout(() => {
